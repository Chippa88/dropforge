...
// ROUTE NOTE: All navigate() paths must match the EXACT page names
// in the Dropforge App Builder. If pages are named "DropforgeLogin",
// "DropforgeOnboarding", etc., update these paths to match.
// The routes below assume pages are named: Login, Onboarding, Pricing
// If they're named DropforgeLogin etc., change /Login → /DropforgeLogin
...
export default function Landing() {
  ...
  const plans = [
    { name: "Starter", price: 29, priceId: "price_1TLu5aPM6gWUtTPVk70mRYFp", products: "20 products/mo", niches: "1 niche",         features: ["AI product generation", "Auto-publisher", "Daily digest email", "Approval queue"],                                         color: "#4ade80", popular: false },
    { name: "Growth",  price: 59, priceId: "price_1TLu5tPM6gWUtTPVzwFiOdyY", products: "75 products/mo", niches: "3 niches",         features: ["Everything in Starter", "Multi-niche support", "Priority trend scanning", "Sales analytics"],                          color: "#6c63ff", popular: true  },
    { name: "Pro",     price: 99, priceId: "price_1TLu65PM6gWUtTPVzonIXm6Z", products: "Unlimited",      niches: "Unlimited niches", features: ["Everything in Growth",  "Auto-replace weak products", "Custom digest schedule", "Bulk auto-publish"],                 color: "#38bdf8", popular: false },
  ];
  ...
  const features = [
    ...
    { icon: "🌐", title: "Private Community",           desc: "Connect with other Dropforge users — share wins, spot trends, ask questions." },
  ];

  // ── Helper: navigate to a page by its Dropforge builder name ──
  // The App Builder sets routes based on what you NAME the page.
  // All our pages are prefixed "Dropforge" so routes are /DropforgeLogin etc.
  function go(page) {
    navigate(`/Dropforge${page}`);
  }
    ...
    <div style={{ background: "#0A0B0F", minHeight: "100vh", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", color: "#fff" }}>
      ...
      <nav style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 60px", borderBottom: "1px solid #1e2030", position: "sticky", top: 0, background: "rgba(10,11,15,0.95)", backdropFilter: "blur(12px)", zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <button onClick={() => go("Login")}      style={{ background: "transparent", border: "1px solid #2d3748", color: "#a0aec0", padding: "8px 20px", borderRadius: 8, cursor: "pointer", fontSize: 14 }}>Log in</button>
          <button onClick={() => go("Onboarding")} style={{ background: "#6c63ff", border: "none", color: "#fff", padding: "8px 20px", borderRadius: 8, cursor: "pointer", fontSize: 14, fontWeight: 600 }}>Get Started</button>
        </div>
      </nav>
      ...
      <section style={{ textAlign: "center", padding: "120px 60px 80px" }}>
        ...
        <h1 style={{ fontSize: 72, fontWeight: 800, letterSpacing: "-2px", lineHeight: 1.05, margin: "0 0 24px" }}>
          ...
          <span style={{ background: "linear-gradient(135deg, #6c63ff, #38bdf8)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>on autopilot.</span>
        </h1>
        ...
        <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={() => go("Onboarding")} style={{ background: "#6c63ff", border: "none", color: "#fff", padding: "16px 36px", borderRadius: 12, cursor: "pointer", fontSize: 16, fontWeight: 700 }}>Start Free Trial →</button>
          <button onClick={() => document.getElementById("features").scrollIntoView({ behavior: "smooth" })} style={{ background: "#13151c", border: "1px solid #1e2030", color: "#a0aec0", padding: "16px 36px", borderRadius: 12, cursor: "pointer", fontSize: 16 }}>See how it works</button>
        </div>
        <p style={{ marginTop: 16, fontSize: 13, color: "#4a5568" }}>Card required · No charge for 7 days · Cancel anytime</p>
      </section>
      ...
      <section id="features" style={{ padding: "100px 60px" }}>
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <p style={{ fontSize: 18, color: "#718096", margin: 0 }}>Six systems working in the background — including a private community you won't find anywhere else.</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24, maxWidth: 1000, margin: "0 auto" }}>
          {features.map((f) => (
            <div key={f.title} style={{ background: "#13151c", border: "1px solid #1e2030", borderRadius: 16, padding: "28px" }}>
      ...
      <section id="pricing" style={{ padding: "80px 60px 120px", background: "#0d0e13" }}>
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <p style={{ fontSize: 18, color: "#718096", margin: "0 0 16px" }}>7-day free trial on every plan. Card required, no charge until day 8.</p>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#13151c", border: "1px solid #1e2030", borderRadius: 20, padding: "6px 16px", fontSize: 12, color: "#718096" }}>
            <span style={{ color: "#4ade80" }}>🔒</span> Secured by Stripe
          </div>
        </div>
        <div style={{ display: "flex", gap: 24, justifyContent: "center", flexWrap: "wrap", maxWidth: 1100, margin: "0 auto" }}>
          {plans.map((plan) => (
            <div
              ...
              style={{ background: "#13151c", border: `1px solid ${plan.popular ? plan.color : hoveredPlan === plan.name ? "#2d3748" : "#1e2030"}`, borderRadius: 20, padding: "36px 32px", width: 300, position: "relative", transition: "all 0.2s ease", transform: plan.popular ? "scale(1.03)" : "scale(1)" }}
            >
              {plan.popular && (
              ...
              <div style={{ background: "#0d2e1a", border: "1px solid #4ade8022", borderRadius: 8, padding: "6px 10px", marginBottom: 16, fontSize: 11, color: "#4ade80", fontWeight: 600 }}>✓ 7-day free trial</div>
              ...
              <div style={{ borderTop: "1px solid #1e2030", paddingTop: 20, marginBottom: 28 }}>
                {plan.features.map((f) => (
                  <div key={f} style={{ fontSize: 13, color: "#718096", marginBottom: 10, display: "flex", gap: 8 }}>
